...
#' @export
vecnet <- function(x, y, covariate_x, covariate_y, max_iter=1000, min_iter = 35, tol=1e-10, log=T, factor=T){
  ...
  param.list <- initialise_param.list(x=x,y=y,covariate_y=covariate_y, covariate_x = covariate_x)

  llik<-score<-tol_vec<-beta_vec<-alpha_vec<-c()
  count=0
  ...
  statistics$alpha_vec = alpha_vec
...
fit.vecnet <- function(vecnet_model,z,covariate_z_remove=NULL){
  ...
  # beta.X_obs <- ((MASS::ginv(as.matrix(t(Z_obs)%*%(Z_obs)))%*%t(Z_obs)%*%(x)))
  # fit.y = fitted + alpha %*% Z_obs %*% beta.X_obs %*% beta.x - alpha %*% x %*% beta.x
  # alpha.inv <- MASS::ginv(as.matrix(t(alpha)%*%(alpha)))%*%t(alpha)
  # beta.inv <- t(beta.X_obs%*%beta.x)%*%MASS::ginv(as.matrix((beta.X_obs%*%beta.x)%*%t(beta.X_obs%*%beta.x)))
  #
  # init_params <- vecnet::initialise_param.list(x = fit.y, y = Z_obs, covariate_y = t(covariate_z.cell_type), covariate_x = t(rep(0,dim(y)[2])), alpha.init =  alpha.inv, beta.init = beta.inv)
  ...
  #
  # Z_align <- as.matrix(init_params$alpha%*%init_params$x%*%init_params$beta + init_params$delta%*%init_params$covariate_y)
...
extract_statistics <- function(param.list){
  ...
  return(list(
    ...
    alpha_inverse = as.matrix(alpha_inverse),
    beta_inverse = as.matrix(beta_inverse)
  )
  )
}

initialise_param.list <- function(x,y,covariate_y,covariate_x,alpha.init=NULL,beta.init=NULL){
  # Prepare y
  yT <- Matrix::t(y)
  y <- Matrix::Matrix(y)
  ...
  if (is.null(beta.init)){
    beta = MASS::ginv(as.matrix(Matrix::t(x)%*%(x)))%*%Matrix::t((x))%*%Matrix::Matrix(matrix(1,nrow=dim(Matrix::t((x)))[2],ncol=dim(y)[1]))%*%(y)
  } else {
    beta = beta.init
  }
  ...
  gamma = (y - alpha%*%x%*%beta - delta%*%covariate_y)%*%Matrix::t(covariate_x%*%beta)%*%MASS::ginv(as.matrix((covariate_x%*%beta)%*%Matrix::t(covariate_x%*%beta)))
...
convert_param_to_S <- function(param.list,alpha_run=T,beta_run=T){
  ...
  if (alpha_run==T){
    ...
    # y <- y - delta%*%covariate_y - intercept_beta%*%beta
...
convert_S_to_param <- function(param.list, alpha_run=T, beta_run=T){
  ...
  # intercept_x <- MASS::ginv(as.matrix(Matrix::t(alpha)%*%alpha))%*%Matrix::t(alpha)%*%(y - alpha%*%intercept_alpha - intercept_beta%*%beta - param.list$delta%*%param.list$covariate_y - gamma%*%covariate_x%*%beta)%*%Matrix::t(beta)%*%MASS::ginv(as.matrix(beta%*%Matrix::t(beta))) - x
  # intercept_alpha <- MASS::ginv(as.matrix(Matrix::t(alpha)%*%alpha))%*%Matrix::t(alpha)%*%(y - alpha%*%(intercept_x+x)%*%beta - intercept_beta%*%beta - param.list$delta%*%param.list$covariate_y - gamma%*%covariate_x%*%beta)
  # intercept_beta <- (y - alpha%*%(intercept_x+x)%*%beta - alpha%*%intercept_alpha  - param.list$delta%*%param.list$covariate_y - gamma%*%covariate_x%*%beta)%*%Matrix::t(beta)%*%MASS::ginv(as.matrix(beta%*%Matrix::t(beta)))

  # param.list$intercept_x <- intercept_x
  # param.list$intercept_alpha <- intercept_alpha
  # param.list$intercept_beta <- intercept_beta

  delta = (y - alpha%*%(x+intercept_x)%*%beta - alpha%*%intercept_alpha - intercept_beta%*%beta - gamma%*%covariate_x%*%beta )%*%Matrix::t(covariate_y)%*%MASS::ginv(as.matrix(covariate_y%*%Matrix::t(covariate_y)))

  gamma = (y - alpha%*%(x+intercept_x)%*%beta - alpha%*%intercept_alpha - intercept_beta%*%beta - delta%*%covariate_y)%*%Matrix::t(covariate_x%*%beta)%*%MASS::ginv(as.matrix((covariate_x%*%beta)%*%Matrix::t(covariate_x%*%beta)))
  ...
  y_hat_alpha_beta <- alpha%*%(as.matrix(x+intercept_x))%*%beta+alpha%*%intercept_alpha+intercept_beta%*%beta + delta%*%covariate_y
