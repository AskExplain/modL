...
#' @export
vecnet_a <- function(x, y, covariate_y, max_iter=1000, tol=1e-10){
  ...
  param.list$iter = count
  param.list$max_iter = max_iter
  param.list$tol = tol
  param.list$tol_vec = tol_vec
  param.list$llik = llik
  param.list$score = score
...
extract_statistics <- function(param.list){
  ...
  aligned_imputation_x <- as.matrix(
    x + alpha_inverse%*%intercept_x%*%beta_inverse + alpha_inverse%*%intercept_beta + intercept_alpha%*%beta_inverse + alpha_inverse%*%delta%*%covariate_y%*%beta_inverse
  )

  aligned_imputation_y <- as.matrix(
    alpha_inverse%*%(y - delta%*%covariate_y)%*%beta_inverse
  )
...
convert_S_to_param <- function(param.list, alpha_run=T, beta_run=T){
  ...
  param.list$delta = (y - alpha%*%(x+intercept_x)%*%beta -  - alpha%*%intercept_alpha - intercept_beta%*%beta )%*%Matrix::t(covariate_y)%*%MASS::ginv(as.matrix(covariate_y%*%Matrix::t(covariate_y)))
  ...
  y_hat_alpha_beta <- alpha%*%(as.matrix(x+intercept_x))%*%beta+alpha%*%intercept_alpha+intercept_beta%*%beta + param.list$delta%*%param.list$covariate_y
