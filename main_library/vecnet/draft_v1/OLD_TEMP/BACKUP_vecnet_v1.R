...
initialise_param.list <- function(x,y,covariate_y){
  ...
  intercept_x <- Matrix::Matrix(1,nrow=dim(y)[1],ncol=dim(y)[2])
...
convert_param_to_S <- function(param.list,alpha_run=T,beta_run=T){
  ...
  if (beta_run==T){
    ...
    y <- y - delta%*%covariate_y - intercept_x - alpha%*%intercept_alpha

    alphax <- alpha%*%x

    t.alphax_y <- Matrix::t(alphax)%*%y
    t.ibeta_y <- Matrix::t(intercept_beta)%*%y
  ...
  if (alpha_run==T){
    ...
    y <- y - delta%*%covariate_y - intercept_beta%*%beta - intercept_x

    xbeta <- x%*%beta

    y_t.xbeta <- y%*%Matrix::t(xbeta)
    y_t.ialpha <- y%*%Matrix::t(intercept_alpha)
...
convert_S_to_param <- function(param.list){
  ...
  intercept_x <- y - alpha%*%intercept_alpha - intercept_beta%*%beta - alpha%*%x%*%beta - delta%*%covariate_y
  intercept_alpha <- MASS::ginv(as.matrix(Matrix::t(alpha)%*%alpha))%*%Matrix::t(alpha)%*%(y - intercept_x - alpha%*%(x)%*%beta - intercept_beta%*%beta - delta%*%covariate_y)
  intercept_beta <- (y - intercept_x - alpha%*%(x)%*%beta - alpha%*%intercept_alpha - delta%*%covariate_y)%*%Matrix::t(beta)%*%MASS::ginv(as.matrix(beta%*%Matrix::t(beta)))

  param.list$intercept_x <- intercept_x
  param.list$intercept_alpha <- intercept_alpha
  param.list$intercept_beta <- intercept_beta

  param.list$delta = (y - alpha%*%x%*%beta - intercept_x - alpha%*%intercept_alpha - intercept_beta%*%beta )%*%Matrix::t(covariate_y)%*%MASS::ginv(as.matrix(covariate_y%*%Matrix::t(covariate_y)))
