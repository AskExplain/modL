...
#' @export
vecnet <- function(x, y, covariate_y, max_iter=1000, min_iter = 35, tol=1e-10){
...
extract_statistics <- function(param.list){
  ...
  aligned_imputation_x <- as.matrix(
    (x + intercept_x + alpha_inverse%*%intercept_beta + intercept_alpha%*%beta_inverse + alpha_inverse%*%delta%*%covariate_y%*%beta_inverse)
  )

  aligned_imputation_y <- as.matrix(
    (y - delta%*%covariate_y - alpha%*%intercept_x%*%beta - alpha%*%intercept_alpha - intercept_beta%*%beta)
  )
  ...
  return(list(
    ...
    aligned_imputation_x = as.matrix(aligned_imputation_x),
    aligned_imputation_y = as.matrix(aligned_imputation_y),
...
initialise_param.list <- function(x,y,covariate_y){
  ...
  # Initialise beta
  beta = internal_xT%*%(rbind(matrix(1,nrow=dim(x)[1]-dim(y)[1],ncol=dim(y)[2]),y))
...
convert_param_to_S <- function(param.list,alpha_run=T,beta_run=T){
  ...
  if (beta_run==T){
    ...
    # y <- y - delta%*%covariate_y - alpha%*%intercept_alpha
...
convert_S_to_param <- function(param.list, alpha_run=T, beta_run=T){
  ...
  intercept_x <- MASS::ginv(as.matrix(Matrix::t(alpha)%*%alpha))%*%Matrix::t(alpha)%*%(y - alpha%*%intercept_alpha - intercept_beta%*%beta - param.list$delta%*%param.list$covariate_y)%*%Matrix::t(beta)%*%MASS::ginv(as.matrix(beta%*%Matrix::t(beta))) - x
  intercept_alpha <- MASS::ginv(as.matrix(Matrix::t(alpha)%*%alpha))%*%Matrix::t(alpha)%*%(y - alpha%*%(intercept_x+x)%*%beta - intercept_beta%*%beta - param.list$delta%*%param.list$covariate_y)
  intercept_beta <- (y - alpha%*%(intercept_x+x)%*%beta - alpha%*%intercept_alpha  - param.list$delta%*%param.list$covariate_y)%*%Matrix::t(beta)%*%MASS::ginv(as.matrix(beta%*%Matrix::t(beta)))

  param.list$intercept_x <- intercept_x
  param.list$intercept_alpha <- intercept_alpha
  param.list$intercept_beta <- intercept_beta

  delta = (y - alpha%*%(x+intercept_x)%*%beta - alpha%*%intercept_alpha - intercept_beta%*%beta )%*%Matrix::t(covariate_y)%*%MASS::ginv(as.matrix(covariate_y%*%Matrix::t(covariate_y)))
