...
subset_param.list <- function(param.list){
  ...
  if (length(gene.prune)==0){
    ...
    gene.prune = c(param.list$cut_gene.prune)[-gene.prune]
    
    # Update top 500 genes
    param.list$cut_gene.prune <- c(gene.prune,setdiff(param.list$all_gene.prune,param.list$cut_gene.prune)[c(1:length(param.list$gene.prune))])
  }
  cell_x.prune = param.list$cell_x.prune
  if (length(cell_x.prune)==0){
    cell_x.prune = c(param.list$cut_cell_x.prune)
  } else {
    cell_x.prune = c(param.list$cut_cell_x.prune)[-cell_x.prune]
    
    # Update top 300 x.cells
    param.list$cut_cell_x.prune <- c(cell_x.prune,setdiff(param.list$all_cell_x.prune,param.list$cut_cell_x.prune)[c(1:length(param.list$cell_x.prune))])
  ...
  if (length(cell_y.prune)==0){
    ...
    cell_y.prune = c(param.list$cut_cell_y.prune)[-cell_y.prune]
    
    # Update top 300 y.cells
    param.list$cut_cell_y.prune <- c(cell_y.prune,setdiff(param.list$all_cell_y.prune,param.list$cut_cell_y.prune)[c(1:length(param.list$cell_y.prune))])
...
convert_S_to_param <- function(param.list, alpha_run=T, beta_run=T, subset_run=F){
  ...
  param.list$gene.prune <- unique(c(gene.prune,which(colSums(alpha.L_check.restart<0.05/prod(dim(alpha.L_check.restart)))==k_dim)))
  param.list$cell_y.prune <- unique(c(cell_y.prune,which(rowSums(beta.K_check.restart<0.05/prod(dim(beta_check.restart)))==1)))
  param.list$cell_x.prune <- unique(c(cell_x.prune,which(rowSums(beta.beta.K_check.restart<0.05/prod(dim(beta.beta.K_check.restart)))==1)))
