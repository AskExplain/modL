...
#'     param.list <- calculate_scores_and_update_params(param.list = param.list, alpha.L_run = T, beta_run = T, delta_run = T)
#'     
...
#'     first.alpha.L <-
#'       Matrix::t(
#'         y %*% log(D%*%Matrix::t(exp(alpha.L%*%x%*%beta+delta%*%covariate_y))) -  y %*% log(D%*%Matrix::t(exp(alpha.L%*%x%*%beta+delta%*%covariate_y))+1)
#'         + x%*%beta%*%exp((1/2)*log(Matrix::t(exp(alpha.L%*%x%*%beta+delta%*%covariate_y)-alpha.L%*%y)^2) - log(D%*%Matrix::t(exp(alpha.L%*%x%*%beta+delta%*%covariate_y))+1))
#'       )
#'     
...
#'   
#'   if (beta_run){
#'     first.beta <- Matrix::t(alpha.L%*%x)%*%Matrix::t(exp((1/2)*log(Matrix::t(exp(alpha.L%*%x%*%beta+delta%*%covariate_y)-alpha.L%*%y)^2) - log(D%*%Matrix::t(exp(alpha.L%*%x%*%beta+delta%*%covariate_y))+1)))
#'     
...
#'   if (delta_run){
#'     
#'     first.delta <- Matrix::t((covariate_y)%*%(exp((1/2)*log(Matrix::t(exp(alpha.L%*%x%*%beta+delta%*%covariate_y)-alpha.L%*%y)^2) - log(D%*%Matrix::t(exp(alpha.L%*%x%*%beta+delta%*%covariate_y))+1))))
#'     
...
#'     
#'     first.D <- ((
#'       ((MASS::ginv(as.matrix(D)))%*%(log(1+MASS::ginv(as.matrix(D))%*%Matrix::t(mu)) - diag(Reduce('+',lapply(c(1:dim(y)[1]),function(y_ID){1/as.matrix(y[y_ID,]+Matrix::diag(D))}))))%*%
#'          Matrix::t((log(1+MASS::ginv(D)%*%Matrix::t(mu)) - diag(Reduce('+',lapply(c(1:dim(y)[1]),function(y_ID){1/as.matrix(y[y_ID,]+diag(D))})))))) +
#'         (Matrix::t(alpha.L%*%y - mu)%*%(alpha.L%*%y-mu)%*%MASS::ginv(as.matrix(MASS::ginv(as.matrix(D))%*%(1+MASS::ginv(as.matrix(D))%*%Matrix::t(mu))%*%(alpha.L%*%y - mu))))
#'     ))
#'     
#'     info.mat.D <- MASS::ginv(as.matrix(first.D%*%Matrix::t(first.D)))
#'     D <- D + info.mat.D%*%first.D
#'     param.list$D <- D
#'     
#'   }
#'   
#'   
#'   param.list$mllik <- sum(alpha.L%*%y%*%((1/2)*log(1e-6+(D%*%Matrix::t(mu))^2)) - alpha.L%*%y%*%((1/2)*log((1+D%*%Matrix::t(mu))^2)) - Matrix::t(MASS::ginv(as.matrix(D))%*%((1/2)*log((1+D%*%Matrix::t(mu))^2)))) + sum(do.call('c',lapply(dim(y)[1],function(y_ID){y[y_ID,]+Matrix::diag(D)}))) + sum((lgamma(1/Matrix::diag(D))))
