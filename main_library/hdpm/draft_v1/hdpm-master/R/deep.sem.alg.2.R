deep.sem.alg.2 <- function(y, numobs, p, r, k, H.list, psi.list, psi.list.inv,
                           mu.list, w.list, ps.y.list, it, eps, main_IDS) {
...
#################################
out <- compute.lik(y, numobs, k, mu.list, H.list, psi.list, w.list, main_IDS)
py <- out$py
ps.y <- out$ps.y
...
for (i in 1:k[1]){
  ...
  Ez <- Ez + w.list[[1]][i]*t(t(H.list[[1]][i,,])%*%MASS::ginv((H.list[[1]][i,,])%*%t(H.list[[1]][i,,])+psi.list[[1]][i,,])%*%t(y - mu.list[[1]][,i]))

  # for (j in 1:k[2]){
  #   Ez <- t(w.list[[1]][i]*w.list[[2]][j]*t(H.list[[2]][j,,])%*%MASS::ginv((H.list[[2]][j,,])%*%t(H.list[[2]][j,,])+psi.list[[2]][j,,])%*%t(Ez1 - mu.list[[2]][,j]))
  # }
...
out <- list(H = H.list, w = w.list, mu = mu.list, psi = psi.list, psi.inv = psi.list.inv,
            likelihood = likelihood, bic = bic, aic = aic, clc = clc,
            s = s, icl_bic = icl.bic, h = h, ps.y = ps.y, ps.y.list = ps.y.list, z.list = z.list, yy = yy, Ez = Ez)
return(out)
}
